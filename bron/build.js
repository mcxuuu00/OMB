...
.cxc .cxc-form .cxc-btn:disabled{opacity:.6;cursor:wait}
.cxc .cxc-form__status{margin-top:16px;font-family:var(--cxc-inter);font-size:14px;line-height:1.5}
.cxc .cxc-form__status:empty{display:none}
.cxc .cxc-form__status.is-ok{color:#1a7f37}
.cxc .cxc-form__status.is-error{color:var(--cxc-red)}
...
<!-- FORMULIER: verstuurd via Web3Forms. Berichten gaan naar het e-mailadres dat bij deze access key hoort
     (in te stellen op https://web3forms.com). -->
<form class="cxc-form" action="https://api.web3forms.com/submit" method="POST">
<input type="hidden" name="access_key" value="1c51af51-0115-4dbc-b071-f93d379a7edb">
<input type="hidden" name="subject" value="Nieuw bericht via cxclusives.nl">
<input type="hidden" name="from_name" value="Website CXclusives">
...
<div class="cxc-field"><label for="cxc-bericht">Bericht*</label><textarea id="cxc-bericht" name="bericht" required></textarea></div>
<button class="cxc-btn" type="submit">Versturen</button>
<p class="cxc-form__status" role="status" aria-live="polite"></p>
</form>
<script>
(function(){
  var f=document.querySelector('.cxc-form');if(!f||!window.fetch)return;
  var st=f.querySelector('.cxc-form__status'),btn=f.querySelector('button[type=submit]');
  f.addEventListener('submit',function(e){
    e.preventDefault();
    btn.disabled=true;btn.textContent='Bezig…';st.className='cxc-form__status';st.textContent='';
    fetch(f.action,{method:'POST',headers:{'Accept':'application/json'},body:new FormData(f)})
      .then(function(r){return r.json();})
      .then(function(d){
        if(d.success){f.reset();st.classList.add('is-ok');st.textContent='Bedankt! Je bericht is verstuurd, we reageren zo snel mogelijk.';}
        else{throw new Error(d.message);}
      })
      .catch(function(){st.classList.add('is-error');st.textContent='Versturen is niet gelukt. Probeer het opnieuw of app ons via +31 647096911.';})
